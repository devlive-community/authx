 ...
 */
@Injectable()
export class SystemLogTypeService implements BaseService {
    ...
    getList(page: CommonPageModel): Observable<CommonResponseModel> {
        ...
        return this.http.get(ApiConfig.API_SYSTEM_LOG_TYPE, options).map(ResponseUtils.extractData);
    }

    register(param: SystemMenuTypeParam): Observable<CommonResponseModel> {
        const options = HttpUtils.getDefaultRequestOptionsByTokenAndJSON();
        return this.http.post(ApiConfig.API_SYSTEM_LOG_TYPE, JSON.stringify(param), options)
            .map(ResponseUtils.extractData);
    }

    update(param: SystemMenuTypeParam): Observable<CommonResponseModel> {
        const options = HttpUtils.getDefaultRequestOptionsByTokenAndJSON();
        return this.http.put(ApiConfig.API_SYSTEM_LOG_TYPE, JSON.stringify(param), options)
            .map(ResponseUtils.extractData);
    }
