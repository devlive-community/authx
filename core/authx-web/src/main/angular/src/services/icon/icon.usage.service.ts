 ...
 */
@Injectable()
export class IconUsageService implements BaseService {
    ...
    getList(page: CommonPageModel): Observable<CommonResponseModel> {
        ...
        return this.http.get(ApiConfig.API_ICON_USAGE, options).map(ResponseUtils.extractData);
    }

    register(param: SystemMenuTypeParam): Observable<CommonResponseModel> {
        const options = HttpUtils.getDefaultRequestOptionsByTokenAndJSON();
        return this.http.post(ApiConfig.API_ICON_USAGE, JSON.stringify(param), options)
            .map(ResponseUtils.extractData);
    }

    update(param: SystemMenuTypeParam): Observable<CommonResponseModel> {
        const options = HttpUtils.getDefaultRequestOptionsByTokenAndJSON();
        return this.http.put(ApiConfig.API_ICON_USAGE, JSON.stringify(param), options)
            .map(ResponseUtils.extractData);
    }
