...
# Set $version to BootStack version (tag will be "bootstack-$version")
if [ -z "$1" ]; then
    version="latest"
else
    version=$1
...
bootstack=$(git -C "$(dirname "$0")" rev-parse --show-toplevel)

if [ -n "$(git -C "${bootstack}" status --porcelain --untracked-files=no)" ]; then
  echo "Working directory is not clean, aborting"
  exit 1
fi

branch=bootstack-$version
if [ "$version" == "latest" ]; then
  branch=master
fi

if [ -z "$(git tag -l "$branch")" ] && [ "$branch" != "master" ]; then
  echo "Version tag does not exist: bootstack-$version"
  exit 1;
fi

tmp=$(mktemp -d -t bootstack-docs-deploy)
target=$tmp/docs
src=$tmp/bootstack
...
git clone -q --depth 1 git@github.com:springstack/bootstack.git "$target"

remote=$(git -C "${bootstack}" config --local --get "remote.$origin.url")
...
if [ -n "$opt_docs" ] ; then
  ...
  # Replace #{BOOTSTACKVERSION} with current BootStack version
  # Escaping of $version is weak here, but it should be fine for typical version strings
  find "$target/docs/$version" -name "*.md" -print0 | xargs -0 perl -pi -e's/\#\{BOOTSTACKVERSION\}/'"$version"'/g'
...
if [ "$version" != "latest" ] && [ -n "$opt_api" ] ; then
  ...
  if [ -z "$opt_dryrun" ]; then
    $s3sync "$src/target/site/apidocs/" "s3://static.bootstack.io/api/$version/"
  fi
fi
...
if [ -z "$opt_dryrun" ]; then
  ...
  curl -u "$GIT_TOKEN:x-oauth-basic" -XPOST -d@- \
     https://api.github.com/repos/springstack/bootstack.github.io/pulls <<EOF
{
...
EOF
    ...
    echo "Please go to https://github.com/springstack/bootstack.github.io and submit a pull request from the \`$updatebranch\` branch"
  fi
